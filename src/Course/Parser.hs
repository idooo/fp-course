...
(|||) p1 p2 = P (\x ->
  let r = parse p1 x
  in if isErrorResult (r) then (parse p2 x) else (r))
...
list1 p = p >>= \h -> (list p) >>= \t -> valueParser (h :. t)
...
smokerParser = ((==) 'y') <$> ((is 'n') ||| (is 'y'))
--smokerParser = (\i -> i == 'y') <$> ((is 'n') ||| (is 'y'))
